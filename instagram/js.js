  ...
  if (replyMsgBtn) {
    replyMsgBtn.addEventListener('click', function() {
      if (contextMsg) {
        ...
        replyPreview.style.borderLeft = 'none';
        replyPreview.style.borderRadius = '0';
        replyPreview.style.padding = '6px 12px';
        ...
        replyTextSpan.textContent = msgText;
        const authorElem = replyPreview.querySelector('#replyPreviewAuthor');
        let author = '';
        if (contextMsg.classList.contains('me')) author = 'Tu';
...
if (chatBody) {
  ...
  if (lastMsg) {
    ...
    if (lastMsg.classList.contains('avatar-message-group')) {
      const lastGroupMsg = lastMsg.querySelector('.message.' + sender + ':last-child');
      if (lastGroupMsg) {
        const lastGroupTextSpan = lastGroupMsg.querySelector('.message-text');
        if (lastGroupTextSpan && lastGroupTextSpan.classList.contains('deleted-message')) {
          isFirstInGroup = true;
        } else {
          isFirstInGroup = false;
        }
      ...
      if (replyTo) {
        let author = '';
        if (replyTo.classList.contains('me')) author = 'Tu';
      ...
      if (sender === 'other') {
        let lastGroup = null;
        if (chatBody) {
          let lastElem = chatBody.lastElementChild;
          if (lastElem && lastElem.classList.contains('avatar-message-group')) {
            lastGroup = lastElem;
          }
        }
